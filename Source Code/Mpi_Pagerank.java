...
public class Mpi_Pagerank {
			...
			int numIterations, double threshold,double[] FinalRVT, Intracomm communicator, int debug)
	{
		...
		//Map<Integer, Double> intermediateRV = new HashMap();
		double [] intermediateRV = new double[totalNumUrls]; 
		//double [][] intermediateRV = new double[totalNumUrls][2];
		//Map<Integer, Double> localRV = new HashMap();
		double [] localRV = new double[totalNumUrls]; 
		//double [][] localRV = new double[totalNumUrls][2]; 
	    ...
	    { 
			...
			dangling = 0.0;
			System.out.println("amIndex.size()  " + amIndex.size());
			System.out.println("adjacencyMatrix.size()  " + adjacencyMatrix.size());
			//for(int i = 0; i <adjacencyMatrix.size();i=i+2)
			for(int i = 0; i <amIndex.size();i=i+2) //am_index = anIndex
            {
				//System.out.println(x);
				source = amIndex.get(i);
				 //FinalRVT.put(source, 0.0);
				System.out.println(" adjacencyMatrix.get(source) " + adjacencyMatrix.get(source));
				...
				for (int j=0;j<outdegree;j++)
				{
					...
					System.out.println("local RVT  "+ targetUrl );
					//System.out.println("Final  RVT  "+ FinalRVT[source]  );
					//System.out.println("Out degree  "+ (double)outdegree  );
					intermediate_rank_value = localRV[targetUrl] +FinalRVT[source] /(double)outdegree;
					//FinalRVT[i][0]=source;
					//localRV[targetUrl][0] = source;
					//FinalRVT.put(source, 0.0);
					//intermediate_rank_value = localRV[targetUrl][1] +FinalRVT[source][1] /(double)outdegree;
					//System.out.println("FinalRVT.get(source) "+ FinalRVT[i][1]);
					//System.out.println("localRV.get(targetUrl) "+localRV[targetUrl][1]+ " "+ targetUrl);
					//if(localRV.get(targetUrl)==null) {
						//localRV.put(targetUrl,0.0);
					//}
					//intermediate_rank_value = localRV.get(targetUrl) +FinalRVT.get(source) /(double)outdegree; /// error comes here
					localRV[targetUrl] = intermediate_rank_value;
					//double value = localRV.get(targetUrl); 
					//value = intermediate_rank_value;
					//localRV.put(targetUrl, value);	
				}
				
				if(outdegree == 0)
				{
					//dangling+= FinalRVT.get(source);
			...
			System.out.println("Total number of URLSSSS "+ totalNumUrls+ " matrix size "+ adjacencyMatrix.size());
			//System.out.println("before "+FinalRVT);
			
			/* Convert 2-d array to single-d array*/
			double [] newLocal = new double [totalNumUrls];
			double [] newFinal = new double [totalNumUrls];
			
			/*for (int x =0; x<totalNumUrls; x++) {
				newLocal[x] = localRV[x][1];
				newFinal[x] = FinalRVT[x][1];
			}*/
			
			
	        /* Distribute pagerank values */ 
			communicator.Allreduce(localRV,0, FinalRVT,0,totalNumUrls, MPI.DOUBLE, MPI.SUM); 
			//communicator.Allreduce(newLocal,0, newFinal,0,totalNumUrls, MPI.DOUBLE, MPI.SUM); 
			System.out.println("After "+FinalRVT);
	       ...
	       for (int i=0;i<totalNumUrls;i++)
	        //	 for (Map.Entry<Integer, ArrayList<Integer>> entry2 : adjacencyMatrix.entrySet()) 
	        		 {
                 
	        		 /*double value = FinalRVT.get(entry2.getKey()); 
	        			value = value+dangling_value_per_page;
	        			FinalRVT.put(entry2.getKey(), value);*/
                        FinalRVT[i]=FinalRVT[i]+dangling_value_per_page;
	        			 // FinalRVT[i][1]=FinalRVT[i][1]+dangling_value_per_page;
                 }
                
	        	 for (int i=0;i<totalNumUrls;i++)
	        	 //for (Map.Entry<Integer, ArrayList<Integer>> entry3 : adjacencyMatrix.entrySet()) 
	        	 {
	        		 /*double value = FinalRVT.get(entry3.getKey()); 
	        		value= damping_factor*value+(1-damping_factor)*(1.0/(double)totalNumUrls);
	        		FinalRVT.put(entry3.getKey(), value);*/
                        FinalRVT[i]= damping_factor*FinalRVT[i]+(1-damping_factor)*(1.0/(double)totalNumUrls);
	        		// FinalRVT[i][1]= damping_factor*FinalRVT[i][1]+(1-damping_factor)*(1.0/(double)totalNumUrls);
	        	 }
	        	 ...
	        	 for(int i=0;i<totalNumUrls;i++)
	        	//for (Map.Entry<Integer, ArrayList<Integer>> entry : adjacencyMatrix.entrySet()) 
	        	 { 
	        		//deltaArray[0]+=Math.abs(intermediateRV.get(entry.getKey())- FinalRVT.get(entry.getKey()));
	        		
	        		 //intermediateRV.putAll(FinalRVT); 
	        		 deltaArray[0] += Math.abs(intermediateRV[i] - FinalRVT[i]);
	        		 //intermediateRV[i] = FinalRVT[i];
	        		// intermediateRV[i][1] = FinalRVT[i][1];
	        	 }
	         ...
	         //communicator.Bcast(newFinal, 0, totalNumUrls, MPI.DOUBLE, 0); 
	        ...
	        /* System.out.println(" totalNumUrls "+ totalNumUrls+ " --- "+  localRV.size());
	         for (Map.Entry<Integer, Double> entry : localRV.entrySet()) {
	        	 localRV.put(entry.getKey(),0.0); 
	         }*/
    ...
    public static void main(String args[]) throws Exception 
    {
		...
		//Map<Integer,Double> FinalRVT = new HashMap();
		//double FinalRVT[][] = new double[totalNumUrls][2];
		if(rank == 0)
			comp_start=System.currentTimeMillis();


		System.out.println(" Total number of urls "+ totalNumUrls+ " : "+adjacencyMatrix.size()+ "rank "+ rank);
		/*for (Map.Entry<Integer, ArrayList<Integer>> entry2 : adjacencyMatrix.entrySet()) {
		
			FinalRVT.put(entry2.getKey(),1.0/ totalNumUrls);
		}*/
		...
		if(rank == 0)
		{
			//for (Map.Entry<Integer, Double> entry : FinalRVT.entrySet()) 
			for(int i=0; i< FinalRVT.length;i++)
			{
				pagerankvalues.put(i,FinalRVT[i]);
				//pagerankvalues.put(i,FinalRVT[i][1]);
				//pagerankvalues.put(entry.getKey(), FinalRVT.get(entry.getKey()));
				//System.out.println(" Size : "+FinalRVT.length);
			}
